...
pub fn print_youtube_messages(video_id: &str, delay_milliseconds: i64) {
    ...
    loop {
        ...
        for message in body.items {
            ...
            let time_to_wait = (published_dt + chrono::Duration::milliseconds(delay_milliseconds)).with_timezone(&Utc) - Utc::now();
            if time_to_wait > chrono::Duration::milliseconds(0) {
                thread::sleep(time::Duration::from_millis(time_to_wait.num_milliseconds().try_into().unwrap()));
        ...
        let time_to_wait = chrono::Duration::milliseconds(body.pollingIntervalMillis.try_into().unwrap());
        if time_elapsed < time_to_wait {
            thread::sleep(time::Duration::from_millis((time_to_wait - time_elapsed).num_milliseconds().try_into().unwrap()));
        }
    }
